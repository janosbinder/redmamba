...
def protein_hash(result):
	myhash = {}
	for row in result:
		if row['id1'] not in myhash:
			myhash[row['id1']] = {}
		if row['id2'] not in myhash[row['id1']]:
			myhash[row['id1']][row['id2']] = {}
		myhash[row['id1']][row['id2']][row['source']] = row['score']
	return myhash
...
def filter_compartments(result):
	...
	filtered = []
	for row in result:
		if row['id2'] in goloc2label:
			filtered.append(row)
	return filtered

	
def get_result(database, table, taxid, source = None):
	if source == None:
		entire = database.query("SELECT id1, id2, source, score FROM %s WHERE type1=%i AND type2=-22;" % (table, taxid)).dictresult()
	else:
		entire = database.query("SELECT id1, id2, '%s' as source, score FROM %s WHERE type1=%i AND type2=-22;" % (source, table, taxid)).dictresult()
...
def max_compartment_score(protein, know, pred, text):
	...
	if protein in know:
		for loc in know[protein]:
			for source in know[protein][loc]:
				score = know[protein][loc][source]
				if loc not in subcell or subcell[loc] < score:
					subcell[loc] = score
	if protein in pred:
		for loc in pred[protein]:
			for source in pred[protein][loc]:
				score = pred[protein][loc][source]
				if loc not in subcell or subcell[loc] < score:
					subcell[loc] = score
	if protein in text:
		for loc in text[protein]:
			for source in text[protein][loc]:
				score = text[protein][loc][source]
				if loc not in subcell or subcell[loc] < score:
					subcell[loc] = score

	translated_labels = {}
	for loc, score in subcell.iteritems():
		translated_labels[goloc2label[loc]] = score
	return translated_labels

def compartment_scores(protein, know, pred, text):
	subcell = {}
	if protein in know:
		for loc in know[protein]:
			for source in know[protein][loc]:
				score = know[protein][loc][source]
				#handle uniprot, goa stuff
				if loc not in subcell:
					subcell[loc] = {}
				if source not in subcell[loc] or subcell[loc][source] < score:
					subcell[loc][source] = score
	if protein in pred:
		for loc in pred[protein]:
			for source in pred[protein][loc]:
				score = pred[protein][loc][source]
	...
	translated_labels = {}
	for loc in subcell:
		for source in subcell[loc]:
			label = "%s_%s" % (source.lower(), goloc2label[loc])
			translated_labels[label] = subcell[loc][source]
	return translated_labels

def color_labels(type1, id1, figure_prefix, subcell):
	kingdom = ""
	if type1 in animals:
		kingdom = "animals"
	elif type1 in fungi:
	...
	for loc in subcell:
		print "%i\t%s\t%s_%s\t%s\t%s" % (type1, id1, figure_prefix, kingdom, loc, color(subcell[loc]/5.0))
...
if __name__ == "__main__":
	...
	for taxid in animals:# + fungi + plants:
		know = get_result(knowledge, "knowledge", taxid, "knowledge")		
		pred = get_result(predictions, "predictions", taxid)
		text = get_result(textmining, "pairs", taxid, "text_mining")
		#text = {} ## DEBUG!!
		...
		for id1 in proteins:
			...
			overview = compartment_scores(id1, know, pred, text)
			color_labels(taxid, id1, "subcell_cell" ,subcell)
			color_labels(taxid, id1, "subcell_overview" ,overview)
